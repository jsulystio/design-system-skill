...
description: >
  ...
  messy Figma file, resolving drift flagged by the linter, and propagating a
  design change (color, spacing, radius, variant) into token edits. Runs the
  repo scripts for mechanical work and reserves the model for judgment. Not for
  structural component code changes, which need an engineer.
---
...
- Read variables on any plan through the Figma Desktop Bridge or the console
  ...
  so fall back to the bridge, do not give up on variables.
...
Pick the flow that matches the request, then follow its reference file.
