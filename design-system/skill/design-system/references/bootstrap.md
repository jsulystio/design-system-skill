...
0. **Ask for the two inputs first.** Before scaffolding or reading anything,
   prompt the person for both, and do not guess either:
   - **The Figma file link** — the working/client file
     (`figma.com/design/:fileKey/...`). You will read variables, screens, and
     components from it, and write its `fileKey` (and `meta.name`) into
     `design-system/figma.config.json`.
   - **The GitHub repo or local path** for this project — where the
     `design-system/` folder should live, and where the app source is. Record
     the source folder(s) in `lint.codePaths` so the `staying-in-sync` page and
     `lint --code` can check code drift.

   If either is missing, ask for it explicitly before continuing.

1. Scaffold the toolkit if this project does not have it yet. If there is no
...
2. Follow `references/read-figma.md` to read the live file:
...
3. For component inventory entries, name each component, its props, states, and
...
5. Present the proposed names, screen coverage, and any drift the linter found
