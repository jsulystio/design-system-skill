...
Trigger: `node scripts/lint.mjs` reported issues, or the person says "resolve
the drift".

1. Run `node scripts/lint.mjs` and read each issue.
...
5. Run `node scripts/build.mjs` then `node scripts/lint.mjs` to confirm clean.
